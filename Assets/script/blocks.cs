...
public class blocks : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.tag == "reversal") // 반전블럭
        {
            ...
            if (Physics2D.gravity == FloorVector)
                Physics2D.gravity = CellingVector;

            else if (Physics2D.gravity == CellingVector)
                Physics2D.gravity = FloorVector;

            else if (Physics2D.gravity == LeftVector)
                Physics2D.gravity = RightVector;

            else if (Physics2D.gravity == RightVector)
                Physics2D.gravity = LeftVector;
        }
